...
public class Flower7 : MonoBehaviour
{
    Score score;
...

    {
        ...
        if (collision.gameObject.tag == "Flower7")
        {

            if (spawnNewBall)
            {
                collision.gameObject.GetComponent<Flower7>().spawnNewBall = false;
    ...
    IEnumerator RespawnBall()
    {
        if (spawnNewBall)
        {
            Score.ActiveScore += 800;
            Instantiate(Flowers[7], new Vector3(ObjX, ObjY, 0), Quaternion.Euler(0, 0, 0));
        }
        Destroy(gameObject);
        yield return null;
    }
